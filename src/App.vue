...
export default {
	...
	async created() {
		...
		if (dataFromLS) {
			...
			this.tickers.forEach((tic) => {
				subscribeToTicker(tic.name, (newPrice) => {
					this.updateTicker(tic.name, newPrice);
				});
			});
		}
	...
	computed: {
		...
		filteredAllCoins() {
			const filteredCoins = this.coins.filter((el) => (el.startsWith(this.ticker.toUpperCase()) ? el : null));
			return filteredCoins.length > 0 ? filteredCoins.sort().splice(0, 4) : false;
		},
	},
	...
	methods: {
		...
		checkTicker() {
			this.tips = this.ticker ? (this.filteredAllCoins ? this.filteredAllCoins : null) : null;
		},

		validateTicker(tipName) {
			this.ticker = tipName;
			this.tickers.forEach((ticker) => {
				if (ticker.name.toUpperCase() === this.ticker.toUpperCase()) {
				...
				<div class="flex">
					<div class="max-w-xs">
						...
						<div class="mt-1 relative rounded-md shadow-md">
							<input
								v-model="ticker"
								v-on:keydown.enter="addTicker(ticker)"
								v-on:input="checkTicker"
								type="text"
								name="wallet"
								id="wallet"
						...
						<div class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap" v-if="tips">
							<span
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer"
								v-for="name in tips"
								v-bind:key="name"
								v-on:click="addTicker(name)"
							>
								{{ name }}
							</span>
						</div>
						<div class="text-sm text-red-600" v-if="tickerError">Такой тикер уже добавлен</div>
					</div>
				</div>
				<button
					v-on:click="addTicker(this.ticker)"
					type="button"
					class="
						my-4
						inline-flex
			...
			<template v-if="tickers.length">
				...
				<div class="flex flex-col">
					...
					<div class="block text-sm font-medium text-gray-700">
						Фильтр:
						<input v-model="filter" class="p-2 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md" />
					</div>
				</div>
				<hr class="w-full border-t border-gray-600 my-4" />
